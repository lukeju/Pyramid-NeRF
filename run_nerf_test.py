...
def batchify_rays(rays_flat, chunk=1024*32, use_depth=False,random_directions=None, background_color=None, **kwargs):
    """Render rays in smaller minibatches to avoid OOM.
    """
    all_ret = {}
    for i in range(0, rays_flat.shape[0], chunk):
        ret = render_rays(rays_flat[i:i+chunk], use_depth=use_depth ,random_directions=random_directions, background_color=background_color, **kwargs)
...
def render(intrinsics, chunk=1024*32, rays=None, depth=None, c2w=None, ndc=True,
    ...
    if use_depth:
        ...
        near_preprocess = depth.copy()
        far_preprocess = depth.copy()
        near_preprocess[near_preprocess < near_bound] = near_bound
        far_preprocess[far_preprocess < near_bound] = far_bound
        near_preprocess = torch.tensor(near_preprocess)
        far_preprocess = torch.tensor(far_preprocess)
        near = near_preprocess - torch.ones(N_rand)*cfg['near_d']
        far = far_preprocess + torch.ones(N_rand)*cfg['far_d']
        near = near.reshape(N_rand,1)
        far = far.reshape(N_rand,1)
    else:
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, chunk,use_depth=use_depth, random_directions=random_directions, background_color=background_color, **kwargs)
...
def train(cfg, log_path, render_cfg_path):
    ...
    for i in trange(start, N_iters):
        ...
        if do_constant_pretraining: # assuming local coordinates
            ...
            all_ret = render(intrinsics, chunk=cfg['chunk_size'], rays=batch_rays, depth=depth_select,
        ...
        if i % cfg['render_testset_interval'] == 0 and i > 0 and i != cfg['iterations']:
