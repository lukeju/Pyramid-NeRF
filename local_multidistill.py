...
def train_and_test_nodes(node_batch, pretrained_nerf,embed_fn,embeddirs_fn, processing_saturated_nodes,pretrain_cfg_res,cfg_res, pretrain_cfg,cfg,global_domain_min, global_domain_max, dev):
...
##################################################################################################################################
    ...
    start=0
    pre_domain_mins = torch.empty(num_pre_networks,3)
    pre_domain_maxs = torch.empty(num_pre_networks,3)
    for voxel_indices in itertools.product(*[range(axis_resolution) for axis_resolution in fixed_resolution]):
        ...
        pre_domain_mins[start, 0] = domain_min[0]
        pre_domain_mins[start, 1] = domain_min[1]
        pre_domain_mins[start, 2] = domain_min[2]
        domain_max = (global_domain_min + (voxel_indices + np.array(1)) * voxel_size)
        pre_domain_maxs[start, 0] = domain_max[0]
        pre_domain_maxs[start, 1] = domain_max[1]
        pre_domain_maxs[start, 2] = domain_max[2]
        start=start+1
...
def train(cfg, log_path):
    ...
    pretrained_nerf, embed_fn, embeddirs_fn=load_pretrained_nerf_model(dev, cfg)
    ...
    while cp['phase'] == 'discovery' and (cp['nodes_to_process'] or cp['saturated_nodes_to_process']) and not killer.kill_now and not restarting_job:
        ...
        multi_network, test_points, errors_per_point, best_errors_per_network, best_errors_per_network_color, best_errors_per_network_density, saturation =\
            train_and_test_nodes(node_batch, pretrained_nerf,embed_fn, embeddirs_fn, processing_saturated_nodes,pretrained_cfg,cfg,pretrained_cfg['discovery'] ,cfg['discovery'], global_domain_min, global_domain_max,dev)
