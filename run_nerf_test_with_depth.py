...
# TODO: move class out of local_distill_gpu2.py (which gets loaded as __main__)
class Node:
    def __init__(self):
        pass
...
def batchify_rays(rays_flat, chunk=1024*32, during_train=False, use_depth=False,random_directions=None, background_color=None, **kwargs):
    """Render rays in smaller minibatches to avoid OOM.
    """
    all_ret = {}
    for i in range(0, rays_flat.shape[0], chunk):
        ret = render_rays(rays_flat[i:i+chunk], during_train=during_train, use_depth=use_depth ,random_directions=random_directions, background_color=background_color, **kwargs)
...
def render(intrinsics, chunk=1024*32, rays=None, during_train=False, depth_near=None, depth_far=None, c2w=None, ndc=True,
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, chunk, during_train=during_train, use_depth=use_depth, random_directions=random_directions, background_color=background_color, **kwargs)
...
def render_path(render_poses, intrinsics, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, enlarge_depth = False):
    ...
    for i, c2w in enumerate(tqdm(c2ws)):
        ...
        if use_fast_sampling:
            ...
            rgb, disp, acc, depth, elapsed_time = render(intrinsics, chunk=chunk, c2w=c2w, **render_kwargs)[:5]
            ...
            if enlarge_depth:
                ...
                depth = depth.view(1, 1, size0, size1)
                depth = m(depth)
                depth = depth.view(size0*scale_factor,size1*scale_factor)
        ...
        if savedir is not None:
            if use_fast_sampling:
                ...
                #filename2 = os.path.join(savedir, 'disp_{:03d}.png'.format(i))
                #imageio.imwrite(filename2, disps[-1])

                filename3 = os.path.join(savedir, 'depth_{:03d}.png'.format(i))
                imageio.imwrite(filename3, depths[-1])
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, background_color=None, pytest=False, no_color_sigmoid=False):
    ...
    #depth_map_path = log_path + '/depth_map.pth'
    #weights_path = log_path + '/weights.pth'
    #z_vals_path = log_path + '/z_vals.pth'

    #torch.save(weights,weights_path)
    #torch.save(z_vals,z_vals_path)
    ...
    pre_sum_weights = weights.sum(axis=1,keepdim=True)
    sum_weights = 1e-10 * torch.ones_like(pre_sum_weights) +  pre_sum_weights
    process_weights = weights/sum_weights
    ...
    depth_map = torch.sum(process_weights * z_vals, -1)
...
def render_rays(ray_batch,
    ...
    if during_train:
        N_samples = cfg['num_samples_per_ray_train']
    else:
        N_samples = cfg['num_samples_per_ray']

    if use_depth:
        N_samples = cfg['num_depth_samples_per_ray']
    ...
    ret = {'rgb_map': rgb_map, 'disp_map': disp_map, 'acc_map': acc_map, 'depth_map': depth_map}  #   return ret
...
def train(cfg, log_path, render_cfg_path):
    ...
    if cfg['render_only']:
        ...
        with torch.no_grad():
            ...
            rgbs, disps, depths = render_path(render_poses, intrinsics, cfg['chunk_size'], render_kwargs_test, gt_imgs=images,
                savedir=testsavedir, render_factor=cfg['render_factor'], enlarge_depth = False)
    ...
    N_pre_iters = None
    if cfg is not None and 'pretrain_iterations' in cfg:
        N_pre_iters = cfg['pretrain_iterations'] + 1
    ...
    for i in trange(start, N_iters):
        ...
        if N_pre_iters is not None and i < N_pre_iters:
            use_depth = False
        else:
            use_depth = True

        if start > N_pre_iters:
            if i == start:
                if has_flag(cfg, 'depth_log_path'):
                    all_depth = torch.load(cfg['depth_log_path'])
                    all_depth = torch.tensor(all_depth)

                    near_bound = near + cfg['near_d']
                    far_bound = far - cfg['far_d']

                    near_preprocess = all_depth.clone()
                    far_preprocess = all_depth.clone()
                    near_preprocess[near_preprocess < near_bound] = near_bound
                    far_preprocess[far_preprocess < near_bound] = far_bound
                    all_near = near_preprocess - torch.ones_like(all_depth) * cfg['near_d']
                    all_far = far_preprocess + torch.ones_like(all_depth) * cfg['far_d']
        ...
        if use_batching:
            ...
            if use_depth:
                #depth = all_depth[img_i,:,:]      #depth of random image
                #depth = depth.reshape(intrinsics.H*intrinsics.W)       # [H*W]
                near_select = all_near[img_i, :, :]  # depth of random image
                near_select = near_select.reshape(intrinsics.H * intrinsics.W)  # [H*W]
                far_select = all_far[img_i, :, :]  # depth of random image
                far_select = far_select.reshape(intrinsics.H * intrinsics.W)  # [H*W]
            ...
            if N_rand is not None:
                ...
                if use_depth:
                    #depth_select = depth[select_inds]     # (N_rand)
                    far_select = far_select[select_inds]  # (N_rand)
                    far_select = far_select.reshape(N_rand,1)
                    near_select = near_select[select_inds]  # (N_rand)
                    near_select = near_select.reshape(N_rand,1)
        ...
        if do_constant_pretraining: # assuming local coordinates
            ...
            if use_depth:
                all_ret = render(intrinsics, chunk=cfg['chunk_size'], rays=batch_rays, during_train=during_train,depth_near=near_select,depth_far=far_select,
                ...
                all_ret = render(intrinsics, chunk=cfg['chunk_size'], rays=batch_rays, during_train=during_train,
                                 verbose=i < 10, retraw=True, use_random_directions=use_random_directions,
                                 random_directions=random_directions, **render_kwargs_train)

            if random_directions is not None:
                all_ret, mean_regularization_term = all_ret                               
            rgb, disp, acc, depth, elapsed_time, extras = all_ret
        ...
        if i % cfg['render_video_interval'] == 0 and i > 0:
            ...
            model.eval()
            with torch.no_grad():
                rgbs, disps,depths = render_path(render_poses, intrinsics, cfg['chunk_size'], render_kwargs_test)
        ...
        # if i == cfg['iterations']:
        #     testsavedir = log_path + '/testset_{:07d}'.format(i)
        #     os.makedirs(testsavedir, exist_ok=True)
        ...
        #         #render_path(torch.Tensor(poses[i_test]).to(device), intrinsics, cfg['chunk_size'], render_kwargs_test, gt_imgs=images[i_test], savedir=testsavedir)
        #
        #         _, _, depths = render_path(torch.Tensor(poses).to(device), intrinsics, cfg['chunk_size'], render_kwargs_test,
        #                     gt_imgs=images, savedir=testsavedir,enlarge_depth = True)
        #
        #     depths_path = log_path + '/depths_{:07d}.pth'.format(i)
        #     torch.save(depths, depths_path)
        #
        #     Logger.write('Saved test set')

        if i == cfg['pretrain_iterations']:
            testsavedir = log_path + '/testset_{:07d}'.format(i)
            os.makedirs(testsavedir, exist_ok=True)
            print('test poses shape', poses.shape)
            model.eval()
            with torch.no_grad():
                ...
                _, _, depths = render_path(torch.Tensor(poses).to(device), intrinsics, cfg['chunk_size'], render_kwargs_test,
                            gt_imgs=images, savedir=testsavedir,enlarge_depth = False)
