...
import lpips
import local_multidistill_multiseg
...
##############################################################################
def load_single_nerf_model(dev, cfg):
    pretrained_cfg = load_yaml_as_dict(cfg['pretrained_single_path'])
    ...
    checkpoint = torch.load(cfg['pretrained_single_checkpoint_path'])
...
##############################################################################
def load_pretrained_nerf_model(dev, cfg, cycle_index):
    ...
    pretrained_nerf, embed_fn, embeddirs_fn, pre_res = create_multinerf(pretrained_cfg)
    res = cfg['fixed_resolution']
    pretrained_nerf = pretrained_nerf.to(dev)
    checkpoint = torch.load(cfg['pretrained_checkpoint_path'])

    res_preres_scale = cfg['res_preres_scale']
    pre_cycle_index = math.floor(cycle_index*pre_res[0]*res_preres_scale/res[0])

    num_pre_networks = pre_res[1]*pre_res[2]

    start = pre_cycle_index*num_pre_networks
    end = (pre_cycle_index+1)*num_pre_networks

    key = 'model_state_dict'
    model_state = checkpoint[key]
    if model_state["pts_linears.0.weight"].ndim > 2:
        for key in model_state.keys():
            matrix = model_state[key]
            if matrix.ndim == 3:
                mat = matrix.permute(0, 2, 1)
                model_state[key] = mat

        model_dict = {
            'pts_linears.0.weight': model_state["pts_linears.0.weight"][start:end,:,:],
            'pts_linears.0.bias': model_state["pts_linears.0.bias"][start:end,:],
            'pts_linears.1.weight': model_state["pts_linears.1.weight"][start:end,:,:],
            'pts_linears.1.bias': model_state["pts_linears.1.bias"][start:end,:],
            'direction_layer.weight': model_state["direction_layer.weight"][start:end,:,:],
            'direction_layer.bias': model_state["direction_layer.bias"][start:end,:],
            'feature_linear.weight': model_state["feature_linear.weight"][start:end,:,:],
            'feature_linear.bias': model_state["feature_linear.bias"][start:end,:],
            'alpha_linear.weight': model_state["alpha_linear.weight"][start:end,:,:],
            'alpha_linear.bias': model_state["alpha_linear.bias"][start:end,:],
            'rgb_linear.weight': model_state["rgb_linear.weight"][start:end,:,:],
            'rgb_linear.bias': model_state["rgb_linear.bias"][start:end,:],
        }
        checkpoint["model_state_dict"] = model_dict

    pretrained_nerf.load_state_dict(checkpoint['model_state_dict'])

    return pretrained_nerf, embed_fn, embeddirs_fn
#######################################################################################################
def create_nerf(cfg):
...
#######################################################################################################
def create_multinerf(cfg):
    res=cfg['fixed_resolution']
    num_network=res[1]*res[2]
    ccfg=cfg['discovery']

    position_num_input_channels, position_fourier_embedding = local_multidistill_multiseg.create_multi_network_fourier_embedding(num_network, ccfg[
        'num_frequencies'])
    direction_num_input_channels, direction_fourier_embedding = local_multidistill_multiseg.create_multi_network_fourier_embedding(num_network, ccfg[
        'num_frequencies_direction'])

    model=local_multidistill_multiseg.create_multi_network(num_network, position_num_input_channels, direction_num_input_channels, 4, 'bmm', ccfg)

    return model,position_fourier_embedding , direction_fourier_embedding , res
