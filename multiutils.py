...
def load_pretrained_nerf_model(dev, cfg):
    ...
    pretrained_nerf, embed_fn, embeddirs_fn = create_multinerf(pretrained_cfg)
    ...
    if model_state["pts_linears.0.weight"].ndim > 2:
        ...
        model_dict = {
            'pts_linears.0.weight': model_state["pts_linears.0.weight"],
            'pts_linears.0.bias': model_state["pts_linears.0.bias"],
            'pts_linears.1.weight': model_state["pts_linears.1.weight"],
            'pts_linears.1.bias': model_state["pts_linears.1.bias"],
            'direction_layer.weight': model_state["direction_layer.weight"],
            'direction_layer.bias': model_state["direction_layer.bias"],
            'feature_linear.weight': model_state["feature_linear.weight"],
            'feature_linear.bias': model_state["feature_linear.bias"],
            'alpha_linear.weight': model_state["alpha_linear.weight"],
            'alpha_linear.bias': model_state["alpha_linear.bias"],
            'rgb_linear.weight': model_state["rgb_linear.weight"],
            'rgb_linear.bias': model_state["rgb_linear.bias"],
        }
...
def create_multinerf(cfg):
    ...
    num_network=res[0]*res[1]*res[2]
    ccfg=cfg['discovery']

    position_num_input_channels, position_fourier_embedding = local_multidistill.create_multi_network_fourier_embedding(num_network, ccfg[
        'num_frequencies'])
    direction_num_input_channels, direction_fourier_embedding = local_multidistill.create_multi_network_fourier_embedding(num_network, ccfg[
        'num_frequencies_direction'])

    model=local_multidistill.create_multi_network(num_network, position_num_input_channels, direction_num_input_channels, 4, 'bmm', ccfg)

    return model,position_fourier_embedding , direction_fourier_embedding
